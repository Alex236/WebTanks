        ...
        console.log("FILL");
            ...
            if (this.isSpawnPoint){
                if(!this.spawnPoints.includes(buttonName[0] + '/' + buttonName[1] + '/6')
                    ...
                    //button.backgroundImage = "SpawnPoint";
                    //this.view.ctx.drawImage("./assets/SpawnPoint.svg", button.x, button.y, button.width*4, button.height*4);
                }
                console.log("isSpawnPoint - " + this.isSpawnPoint);
            }
            else {
                //arenaControls.map(control => this.clickedCells(control, Number(buttonName[0]), Number(buttonName[1])));
                //arenaControls.map(control => this.arrayOfFillingCells(control));
                //this.splitBlocks(arenaControls);
                button.name = buttonName[0] + "/" + buttonName[1] + "/" + this.activeBrush[1];
            }
            //this.drawMaxGamers();

            console.log(button);
            console.log(this.maxGamers);
            
            console.log(arenaControls);
            //console.log(arenaControls.filter(x => Number(x.name.split('/')[2]) != 0));
        }
